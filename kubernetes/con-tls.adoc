...
Both conditions must be met for the connection to be secure.
...
`skupper-local-client`:: Contains the key, certificate, and the CA certificate used by the `skupper-service-controller` and the `skupper-flow-collector`.
`skupper-local-server`:: Contains the key, certificate, and the CA certificate used by the skupper router.
