...
The example backend takes a name as input and returns a greeting.
...
[source,bash,term=local]
----
 curl --verbose -H "Content-type: application/json" -d '{"name": "Hubert"}' localhost:8080/api/hello
----
...
[source,bash,term=local]
----
{"text":"Hi, Hubert.  I am Heroic Hardware (<machine name>).","name":"Heroic Hardware"}
----
