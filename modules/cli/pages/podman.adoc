...
* On Kubernetes sites, `skupper service status` shows details about original service location.
This information might not be available from Podman sites.
...
--
----
$ podman run -it --network=skupper --name=myubi ubi8/ubi curl backend:8080/api/health
...
. Determine the IP address of the `hello-world-backend` container:
+
----
$ podman inspect --format='{{.NetworkSettings.Networks.skupper.IPAddress}}' hello-world-backend
...
+
----
$ skupper expose host 10.89.0.10 --address backend --port 8080 
----
