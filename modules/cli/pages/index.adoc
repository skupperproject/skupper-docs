...
If you install the policy system as described in xref:policy:index.adoc[], the message becomes `Skupper is enabled for namespace "west" in interior mode (with policies)`.
...
For more information about protecting access to the {service-network}, see xref:cli:tokens.adoc[].
====
--
...
For a full list of options, see the xref:cli-reference:skupper.adoc[Kubernetes] and xref:cli-podman:skupper.adoc[Podman] reference documentation.
