...
For TCP, the exact usage level of a link is determined by the number of open connections.
When the number of open connections exceeds the cost, the next request is likely to open a connection to a different target.
