...
In this case, you can specify a cost of greater than `1` to reduce the usage of the link.
...
Since release 2.2.1, a local service can be balanced with a remote service.
For example, if you deploy a web server  in site A and B, and bind both to a service `http-server`, client requests from site A will be balanced equally with responses from site A and B when the service is under load.
If only a few requests are sent, the responses come from site A.
