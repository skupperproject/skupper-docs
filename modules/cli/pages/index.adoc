...
* If you have services distributed across different sites, you might want a calling service to favor a particular target or link.
...
* Cost acts as a threshold for using a path in the network.
...
* When there are a number of paths, traffic flows on the lowest cost path until the number of connections exceeds the cost of an alternative path.
After this threshold of open connections is reached, new connections use the alternative path
