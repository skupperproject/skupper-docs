...
{skupper-name} allows you expose services from other namespaces by default. 
You can expose other resources, for example deployments if you create a site as described in this section. 
...
. Create a site with cluster permissions if you want to expose a Kubernetes resource other than a service:
+
--
----
...
.. If you want to expose a service, for example, if you did not create a site with `--enable-cluster-permissions`:
+
--
----
...
.. If you want to expose a Kubernetes resource, that is, you created a site with `--enable-cluster-permissions`:
+
--
----
