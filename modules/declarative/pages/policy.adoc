...
For example, the following policy CR fully allows all {skupper-name} capabilities on all namespaces, except for exposed resources.
The policy allows incoming links from any domain ending in `example.com` and adds 'deployment/nginx' to the list of allowed resources on all namespaces.
...
`allowedExposedResources`:: Specify one or more permitted names of service bindings allowed on the {service-network} from the namespaces specified. 
