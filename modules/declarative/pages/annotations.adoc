...
This is the only annotation that is required and is equivalent to the `skupper expose` command.
* `skupper.io/port` - the ports for the service on the {service-network}. 
* `skupper.io/proxy` - the protocol you want to use, `tcp`, `http` or `http2`, defaults to `tcp`.
