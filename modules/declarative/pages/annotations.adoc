...
[NOTE]
====
When exposing services, rather than other resources like deployments, you must use the `skupper.io/target` annotation.
For example, if you want to expose the `backend` service:
----
$ kubectl annotate service backend "skupper.io/address=van-backend" "skupper.io/port=8080" "skupper.io/proxy=tcp"
----
====
