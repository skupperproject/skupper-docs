...
. If Skupper sites that were not created by `cluster-admin` exist in the cluster, you must grant permissions to read Skupper policies to developers to avoid that site being blocked from the {service-network}.
+
--
For each site namespace:
