...
You can also apply the {policy-system} to a cluster, all {service-network} communication to and from that cluster is denied, and you can specify granular policies to allow your {service-network} operate.

{skupper-name} provides default, built-in security that scales across clusters and clouds, for example xref:network-policy[] and xref:tls[].
