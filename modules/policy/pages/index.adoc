...
NOTE: A cluster administrator can apply each {policy-system} setting to one or more namespaces.
...
`allowIncomingLinks`:: Specify `true` or `false` to determine whether other sites can create links to the specified namespaces.
...
Install the {policy-system} CRD enables a cluster administrator to enforce policies for {service-network}s.
