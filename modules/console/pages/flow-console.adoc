...
By default, when you create a {skupper-name} site, you do not the Skupper console.
...
----
+
NOTE: You must enable the console on the same site as you enable the flow collector.
...
* Components - services that are exposed on the {service-network}
* Processes - running processes relating to exposed services.
* Addresses - services and port numbers relating to exposed services
* Network - visualizations of sites, components and processes
...
image::{img-loc}skupper-adservice.png[]


. Deploy the https://github.com/skupperproject/skupper-example-grpc[microservices example], using the associated README.

. Navigate to the Skupper console using the instructions in xref:accessing-console[]

. Click the *Network* menu item.
+
--
The *Sites* tab shows how the sites in the {service-network} are linked.
For example, you can see that no token was created on the *private1* site but it is linked to *public1* and *public2*.
Clicking on a site displays the detail from the *Sites* tab.

The *Components* tab shows the services you exposed and the components that call those services.
For example, the *frontend* component is not an exposed service, but a process that calls the exposed services.

NOTE: Not only are services exposed on the {service-network} displayed, but the components that call the exposed services are also represented.

Clicking on a component displays the detail from the *Components* tab.

The *Processes* tab shows detail about the services you exposed, which is useful when services are scaled.
For example, if a service has three replicas, you can monitor each replica.
Clicking on a process displays the detail from the *Processes* tab.
