...
If using username based console authentication, there is also a *skupper-console-users* secret that contains the username(s) and password(s) that can be used to access the console. A username and password can be specified using `skupper init --console-user <username> --console-password <password>`. By default, an `admin` user is generated with a generated password.
