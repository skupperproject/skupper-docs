...
NOTE: Running `skupper link status` on a connected site only produces output if a token was used to create a link.   
--
...
* There are two sites on the {service-network}, `east` and `west`.
...
* The services available on the {service-network}, including port, for example, `backend:8080`. 
--
...
Use the following command to create a loadbalancer:
