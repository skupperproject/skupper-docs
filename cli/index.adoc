...
In this case, you can later expose a different deployment or statefulset from another site using the same address (routing key).
...
If you require stable network identifiers on the {service-network}, use the `--headless` option.
...
NOTE: When you use the `--headless` option, you can only expose the service from the current cluster.
