...
----
$ skupper status
Skupper is enabled for "<username>" with site name "<machine-name>-<username>". It is not connected to any other sites. It has no exposed services.
----
...
== Deleting a Skupper podman site

If you are no longer using a podman site, you can delete that site and recover resources.

.Prerequisites

* A running Skupper podman site
...
. Make sure you are only removing resources you no longer require.
+
--
When you expose a container from a podman site, Skupper adds a `application=skupper` label to the container. 
When you delete a podman site, all podman resources labeled with `application=skupper` are removed. 
